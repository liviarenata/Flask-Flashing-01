...
def create():
  ...
  if len(title) > 10:
    flash('O Título não pode ter mais do que 10 caracteres', 'error')
    return redirect('/')
...
def update(id):
  ...
  if len(title) > 10:
    ...
    flash('TODO atualizado com sucesso!', 'success')
...
def signup():
  ...
  if user:
    flash('Este e-mail já existe em nosso sistema', 'error')
...
def signin():
  ...
  user = Users.query.filter_by(email=email_input).first()
  if not user:
    flash('E-mail e/ou Senha Invalidos', 'error')
    return redirect('/login')
  ...
  flash(f'Bem-Vindo, {user.name}', 'info')
  return redirect('/')
